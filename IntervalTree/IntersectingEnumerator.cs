...
{
	/// <summary>Class for enumerating all intersecting elements in a <see cref="IntervalTree{TElement,TEndpoint}"/> for a specified interval.</summary>
		...
		where TEndpoint : IComparable<TEndpoint>
	{
		...
		internal IntersectingEnumerator(IntervalTree<TElement, TEndpoint> tree, IInterval<TEndpoint> interval, bool asEnumerator = false)
		...
		/// <param name="tree">Original tree enumerating.</param>
		/// <returns>This instance cloned.</returns>
		protected override IEnumerator<TElement> CloneAsEnumerator(IntervalTree<TElement, TEndpoint> tree)
		{
			return new IntersectingEnumerator<TElement, TEndpoint>(tree, interval, true);
		}
		...
		protected override bool GoLeft(IntervalNode<TElement, TEndpoint> node)
		{
			...
			return Comparer.Default.Compare(node.Left.Max, interval.Start) >= 0; // TODO: intersect vs adjacent?
		}
		...
		protected override bool GoRight(IntervalNode<TElement, TEndpoint> node)
		{
			...
			return Comparer.Default.Compare(node.Interval.Start, interval.End) <= 0; // TODO: intersect vs adjacent?
		}

		/// <summary>Determine whether to stop at the specified node. If true, will set to <see cref="InOrderEnumerator{TElement,TEndpoint}.Current"/> and return.</summary>
